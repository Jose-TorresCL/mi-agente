...
def test_telegram_interface_imports_correctamente():
    ...
    try:
        import app.telegram_interface as telegram_interface  # noqa: F401
...
def test_sessions_typing_es_correcto():
    ...
    from app.telegram_interface import UserSession
...
def test_load_sessions_devuelve_dict_si_archivo_no_existe(tmp_path, monkeypatch):
    ...
    from app.telegram_interface import _load_sessions

    # Monkeypatch para usar tmp_path
    fake_sessions_file = tmp_path / "telegram_sessions.json"
    monkeypatch.setattr("app.telegram_interface.SESSIONS_FILE", fake_sessions_file)
...
def test_persist_sessions_crea_archivo(tmp_path, monkeypatch):
    ...
    from app.telegram_interface import _persist_sessions
...
def test_persist_sessions_maneja_errores_silenciosamente():
    ...
    with patch("app.telegram_interface.SESSIONS_FILE") as mock_file:
...
def test_start_handler_responde_mensaje():
    ...
    from app.telegram_interface import start
...
def test_reset_handler_existe_y_es_callable():
    ...
    from app.telegram_interface import reset
...
def test_main_verifica_token():
    ...
    from app.telegram_interface import main

    # Si TOKEN es None, main() debería lanzar ValueError
    with patch("app.telegram_interface.TOKEN", None):
...
def test_main_construye_app_correctamente():
    ...
    with patch("app.telegram_interface.TOKEN", "test-token"):
        with patch("app.telegram_interface.ApplicationBuilder") as mock_builder:
            mock_app = MagicMock()
            ...
            # No debería lanzar
            try:
                with patch("app.telegram_interface.app.run_polling"):
                    pass
            except Exception as e:
...
def test_embed_model_es_string():
    ...
    from app.telegram_interface import EMBED_MODEL
...
def test_sessions_file_path_es_valido():
    ...
    from app.telegram_interface import SESSIONS_FILE
...
def test_load_sessions_maneja_archivo_ilegible(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr("app.telegram_interface.SESSIONS_FILE", _UnreadableFile())

    # Mock de exists para que devuelva True
    with patch("app.telegram_interface.SESSIONS_FILE.exists", return_value=True):
        result = _load_sessions()
        assert result == {}
...
def test_reset_handler_reinicia_historial(monkeypatch):
    ...
    import app.telegram_interface as telegram_interface
