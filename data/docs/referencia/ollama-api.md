...
Additional navigation options
     ...
     FilesExpand file tree mainBreadcrumbsollama/docs/api.mdCopy pathBlameMore file actionsBlameMore file actions Latest commit HistoryHistoryHistory1931 lines (1552 loc) · 54.8 KB mainBreadcrumbsollama/docs/api.mdTopFile metadata and controlsPreviewCodeBlame1931 lines (1552 loc) · 54.8 KBRawCopy raw fileDownload raw fileOutlineEdit and raw actionsAPI
...
WarningThese parameters are experimental and may change in future versions.
...
ImportantIt's important to instruct the model to use JSON in the prompt. Otherwise, the model may generate large amounts whitespace.
...
Request (JSON mode)
ImportantWhen format is set to json, the output will always be a well-formed JSON object. It's important to also instruct the model to respond in JSON.
...
WarningImage generation is experimental and may change in future versions.
...
See the Generate a completion section for the full API documentation. The experimental image generation parameters (width, height, steps) are documented there.
Example
Request
curl http://localhost:11434/api/generate -d '{
